...
class VisionTfliteDataSource {
  ...
  Future<void> init() async {
    ...
    try {
      ...
      _interpreter = await Interpreter.fromAsset(_modelAsset).timeout(

        const Duration(seconds: 5),

        onTimeout: () => throw TimeoutException('TFLite model load timeout'),
  ...
  Future<Map<String, double>> classify({
    ...
    await _ensureReady().timeout(const Duration(seconds: 6));
